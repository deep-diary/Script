...
%       cfg：模型配置文件
% 返回：None
% 范例：changeCfgRef('TmComprCtrl', 'TmVcThermal_Configuration_sub'),
...
%     cfg = 'TmVcThermal_Configuration_sub';
    ...
    try
        ...
        warning(['"TmVcThermal_Configuration_sub.mat"' 'is not fund!']);
    end
    h = load_system(mdName);

    k=getConfigSet(mdName,refName);%查找模型所以的模型设置引用
    if isempty(k)
        configRef=Simulink.ConfigSetRef;%新建模型设置
        set_param(configRef,'Name',refName);%将模型引用命名
        set_param(configRef,'SourceName',cfg);%设置模型引用源
        attachConfigSet(mdName,configRef);%将模型引用与模型关联
    else
        set_param(k,'SourceName',cfg);%设置模型引用源
    end
    setActiveConfigSet(mdName,refName);%激活模型引用
%     set_param(mdName,'SaveWithDisabledLinksMsg','none')
