...
%   'modelCacheDir'  - 模型缓存目录（默认：与 ARXML 同目录下 arxml_models_<arxmlBase>）
%   'excelFile'      - Excel 输出文件路径（默认：与 ARXML 同目录 <arxmlBase>_PortsInfo.xlsx）
...
[arxmlFolder, arxmlBase, ~] = fileparts(ArxmlFileName);
...
if isempty(modelCacheDir)
    modelCacheDir = fullfile(arxmlFolder, ['arxml_models_' arxmlBase]);
end
if isempty(excelFile)
    excelFile = fullfile(arxmlFolder, [arxmlBase '_PortsInfo.xlsx']);
end
