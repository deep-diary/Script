function createHarnessStepfromtable(harnessName,lastStp, varargin)
%%
% 目的: 为模块追加测试用例
% 输入：
% 返回：None
...
%     addParameter(p,'lastStp','CompDischargeTProtect');      % 设置变量名和默认参数
    ...
    addParameter(p, 'HoldTime', {{1,1},{1,1,1}});%%hjm,新增HoldTime
    ...
    'Name',lev1Arry{i});
        ...
        for j=1:length(lev)
            if j == length(lev)
                ...
                HoldTime_loc = HoldTime{i}{j};
                
                outAction = createHarnessVerifyStatementfromtable(harnessName,outputName,outputValue,HoldTime_loc, ...
                    'mask',mask, ...
                    'tolerance',tolerance,...
                    'waitTime',waitTime);
                %%
                for aa=1:length(inputValue)
                    if ischar(inputValue{aa})
                        inputValue{aa}=str2num(inputValue{aa});
                    end
                end
                %%
                ...
                if isIncludeArr
                    ...
                    for k=1:inTypes
                        inAction = createHarnessInputStatementfromtable(inputNames,inputPkg{k});
                    ...
                    inAction = createHarnessInputStatementfromtable(inputNames,inputValue);
