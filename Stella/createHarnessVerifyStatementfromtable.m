function outAction = createHarnessVerifyStatementfromtable(harnessName,names,values,HoldTime,varargin)
%%
% 目的: 为模块追加测试用例
% 输入：
% 返回：None
...
%%
    ...
    waitTime = HoldTime{1};%%%%%%这对吗？
    if ischar(waitTime)
        HoldTime = str2num(waitTime);
    else
        HoldTime = waitTime;
    end
...
%     values = {10, 5.0};
    ...
    for i = 1:len
        ...
        if maskFlag==false || mask(i) == 0  % there is no need to mask%%hjm,把waittime切换成HoldTime
            verify = sprintf([...
                'verify(et<%f||et>%f||abs(%s-single(%s))<%f,' ...
                '''SimulinkTest:%s'',' ...
                '''the result is %%f'',single(%s));\n'],HoldTime,HoldTime, outputValue, outputName,tolerance, harnessName,outputName);
        else
            verify = sprintf([...
                'verify(et<%f||et>%f||abs(%s - bitand(%s,%d))<%f,' ...
                '''SimulinkTest:%s'',' ...
                '''the result is %%f'',single(%s));\n'],HoldTime,HoldTime, outputValue, outputName,mask(i), tolerance, harnessName,outputName);
        end
