function createHarnessNewfromtable(path, varargin)
%%
% 目的: 为模块创建测试用力
% 输入：
% 返回：None
...
%     lev2Arry = {{'SM1','SM2'},{'SM4','SM5','SM6'}};
    ...
    %% 添加测试步骤
    createHarnessStepfromtable(harnessName,lastStp, ...
        'tolerance',tolerance,...
        'waitTime',waitTime,...
        'lev1',lev1Arry, ...
        'lev2',lev2Arry, ...
...
%     end
    ...
    for i=1:length(lev1Arry)-1
        ...
        for j=1:length(lev)-1
            ...
            [isIncludeArr, inputPkg] = findHarnessInputType(inputValue);
            tranTime = ['et>' num2str(HoldTime{i}{j}{1})];
            if isIncludeArr
                % 二级转移逻辑
                sltest.testsequence.addTransition(seqPath,...
                from,'endTest2 == true',to) 

                inTypes = length(inputPkg);%%hjm,trans语句加入
                for k=1:inTypes
