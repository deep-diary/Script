function createHarnessfromtable(path, varargin)
...
%   版本: 1.1
    ...
    try
        ...
        addParameter(p, 'HoldTime', {{0.05,0.05},{0.05,0.05,0.05}}, @iscell);%%hjm,新增HoldTime
        ...
        HoldTime = p.Results.HoldTime;%%hjm,新增HoldTime
        usingAllPorts = p.Results.usingAllPorts;
        logValue = p.Results.logValue;

        %% 输入数值转换
        % 字符串转换成数值
        loc_size = length(size(inValue));
        switch loc_size
            case 1
                for i=1:length(inValue)
                    if ischar(inValue{i})
                        inValue{i} = str2num(inValue{i});
                    end
                end
            case 2
                for i=1:length(inValue)
                    ...
                    for j=1:length(lev1Value)
                        if ischar(lev1Value{j})
                            inValue{i}{j} = str2num(lev1Value{j})
                        end
                    end
                end
            otherwise
                ...
                %% hjm,输入HoldTime数值转换
        % 字符串转换成数值
        loc_size = length(size(HoldTime));
        switch loc_size
            case 1
                for i=1:length(HoldTime)
                    if ischar(HoldTime{i})
                        HoldTime{i} = str2num(HoldTime{i});
                    end
                end
            case 2
                for i=1:length(HoldTime)
                    lev1HoldTime = HoldTime{i}
                    for j=1:length(lev1HoldTime)
                        if ischar(lev1HoldTime{j})
                            HoldTime{i}{j} = str2num(lev1HoldTime{j})
                        end
                    end
                end
            otherwise
                for i=1:length(HoldTime)
                    lev1HoldTime = HoldTime{i}
                    for j=1:length(lev1HoldTime)
                        lev2HoldTime = lev1HoldTime{j}
                        for k=1:length(lev2HoldTime)
                            if ischar(lev2HoldTime{k})
                                HoldTime{i}{j}{k} = str2num(lev2HoldTime{k});
                            end
                        end
                    end
                end
        ...
        if strcmp(lastStep, 'Initialize')
            % 新创建测试用例
            createHarnessNewfromtable(path, ...
                'mask', mask, ...
                'tolerance', tolerance, ...
                'waitTime', waitTime, ...
                'HoldTime', HoldTime, ...
