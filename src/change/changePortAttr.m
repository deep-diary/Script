function changePortAttr(path, varargin)
%CHANGEPORTATTR 修改端口模块的属性
%   CHANGEPORTATTR(PATH) 修改指定路径下所有端口模块的属性
%   CHANGEPORTATTR(PATH, 'Type', TYPE) 指定要修改的端口类型
%   CHANGEPORTATTR(PATH, 'Type', TYPE, PROPERTY, VALUE) 设置指定属性
%
%   输入参数:
...
%
%       % 只修改Inport模块的标签位置
%       changePortAttr(gcs, 'Type', 'in', 'ShowName', 'off')
%
%       % 修改多个属性
%       changePortAttr(gcs, 'Type', 'out', 'ShowName', 'off', 'PortNumber', '1')
%
%       % 修改模块尺寸
...
%   日期: 20250609
    ...
    % 验证type参数
    if ~ismember(type, {'in', 'out', 'both'})
        error('Type参数必须是''in''、''out''或''both''');
    end
