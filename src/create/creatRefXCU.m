function creatRefXCU(modelName, varargin)
%CREATEREFXCU 创建XCU配置的模型引用
%   creatRefXCU(modelName) 为指定的模型创建XCU配置的模型引用。
%
%   输入参数:
...
%
%   示例:
%       creatRefXCU('TmSovCtrl')
%       creatRefXCU('TmComprCtrl', 'SLDD', 'custom_DD.xlsx')
%
%   作者: 葛维冬 (Blue Ge)
%   日期: 2025-06-09
%   版本: 1.1
    ...
    try
        ...
        %% 加载SLDD文件
%         slddPathArch = 'TmSwArch_DD_XCU.xlsx';
        
        % 指定模型SLDD路径
        if strcmp(slddPath, 'None')
            slddPathMod = [modelName, '_DD_XCU.xlsx'];
        else
...
%         findSlddLoad(slddPathArch);
        ...
        %% 配置采样时间参数
        cTmSwArch_t_s32SampleTime = GCSC.Parameter(0.05);
        cTmSwArch_t_s32SampleTime.DataType = 'single';
        
        %% 创建或打开引用模型
        refModelName = 'subModRefXCU';
        ...
        fprintf('XCU模型引用创建完成: %s\n', modelName);
        
    catch ME
        error('创建XCU模型引用时发生错误: %s', ME.message);
    end
end
