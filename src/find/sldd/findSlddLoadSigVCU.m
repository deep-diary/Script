function loadedCnt = findSlddLoadSigVCU(path, varargin)
%%
% 目的: 导入sldd表格中的Signal
% 输入：
%       path: sldd path
% 返回： loadedCnt: 成功导入的数据条数
% 范例： findSlddLoadSigVCU('TmComprCtrl_DD_VCU.xlsx')
% 作者： Blue.ge
% 日期： 20240201
%% 
    clc
    ...
    %% 导入数据
    try
        opts_alt = detectImportOptions(path,'ExpectedNumVariables',17,'Sheet',"Signals");
        ...
        opts_alt = detectImportOptions(path,'ExpectedNumVariables',14,'Sheet',"Signals");
...
%             opts = setvaropts(opts, ["ModelName", "PortType", "Name", "DataType", "CustomStorageClass", "DefinitionFile", "RTE_Interface", "Details", "ValueTable", "Unit", "IniValue", "DataTypeSelect", "CustomStorageClassSelect", "DefinitionFile1","Min","Max"], "EmptyFieldRule", "auto");
    ...
    for i=1:sz(1)
        ...
        % 判断是否需要排除这个数据
        if ismember(sig_attributes.PortType, exclude)
            continue
        end

        sig_create_station = strcat(sig_attributes.Name, '=' ,'GeelyPack.Signal',';');
...
% 
%         sig_owner_assign = strcat(sig_attributes.Name,'.CoderInfo.CustomAttributes.Owner','=','"',sig_attributes.ModelName,'"',';');
%         sig_DefinitionFile_assign = strcat(sig_attributes.Name,'.CoderInfo.CustomAttributes.DefinitionFile','=','"',sig_attributes.DefinitionFile,'"',';');
%         sig_Header_assign = strcat(sig_attributes.Name,'.CoderInfo.CustomAttributes.HeaderFile','=','"',sig_attributes.DefinitionFile1,'"',';');
...
%         if isempty(strfind(sig_attributes.CustomStorageClass,'_IMP'))
%             evalin('base',sig_owner_assign);
%             evalin('base',sig_DefinitionFile_assign);
%         end
