classdef smart_thermal < matlab.apps.AppBase
    ...
    properties (Access = public)
        ...
        SearchEditFieldLabel            matlab.ui.control.Label
        ...
        SetModelPathButton              matlab.ui.control.Button
        ModelPathEditField              matlab.ui.control.EditField
        ModelPathEditFieldLabel         matlab.ui.control.Label
        ...
        FindModelInfoButton             matlab.ui.control.Button
        DCMEditField                    matlab.ui.control.EditField
        DCMEditFieldLabel               matlab.ui.control.Label
        ModelInfoTextArea               matlab.ui.control.TextArea
        SetDCMButton                    matlab.ui.control.Button
        SystemEnumPanel                 matlab.ui.container.Panel
        EnumInfoTextArea                matlab.ui.control.TextArea
        EnumInfoTextAreaLabel           matlab.ui.control.Label
        EnumListListBox                 matlab.ui.control.ListBox
        EnumListListBoxLabel            matlab.ui.control.Label
    ...
    methods (Access = private)
        ...
        function startupFcn(app)
            ...
            app.MCUTypeDropDown.Items = config.com_used_controllers;s
...
%             
        ...
        function findModInfoButtonPushed(app, event)
...
%           
        ...
        function findEnumTypesButtonPushed(app, event)
            ...
            if info.enumCount == 0
                app.LogTextArea.Value = "未检测到已加载的枚举类。";
                return;
            end

            % 在列表中显示枚举变量
            app.EnumListListBox.Items = info.enumClassNames;

            % 记录日志，后续可直接用于“枚举列表 -> 枚举值明细”联动。
            logStr = sprintf("已加载枚举类数量: %d\n\n%s\n", info.enumCount, info.displayText);
            app.LogTextArea.Value = logStr;
        end
        ...
        function ArxmlTabButtonDown(app, event)
            ...
            app.SignalsListBox.Items = result.allISignals;
            % 载入所有组件模型
        ...
        function SignalsListBoxValueChanged(app, event)
            ...
            % 记录日志，便于确认操作结果
            inModels = string(signals.inputConsumers.ComponentName);
            outModels = string(signals.outputProviders.ComponentName);
            if isempty(inModels)
                inText = '(无)';
            else
                inText = strjoin(cellstr(inModels), newline);
            end
            if isempty(outModels)
                outText = '(无)';
            else
                outText = strjoin(cellstr(outModels), newline);
            end

            logstr = sprintf('当前信号: %s\n输入模型(%d):\n%s\n---------\n输出模型(%d):\n%s\n', ...
                value, numel(inModels), inText, numel(outModels), outText);
            app.LogTextArea.Value = logstr;
        ...
        function ComponentsListBoxValueChanged(app, event)
            ...
            components = findComponentPortsFromPortsInfo(value);
            app.SigInListBox.Items = components.inputPorts.PortName;
            app.SigOutListBox.Items = components.outputPorts.PortName;

            % 记录日志，便于确认操作结果
            inSignals = string(components.inputPorts.PortName);
            outSignals = string(components.outputPorts.PortName);
            if isempty(inSignals)
                inText = '(无)';
            else
                inText = strjoin(cellstr(inSignals), newline);
            end
            if isempty(outSignals)
                outText = '(无)';
            else
                outText = strjoin(cellstr(outSignals), newline);
            end

            app.LogTextArea.Value = sprintf('当前模型: %s\n输入信号(%d):\n%s\n---------\n输出信号(%d):\n%s\n', ...
                value, numel(inSignals), inText, numel(outSignals), outText);
        end
        ...
        function ModelOutListBoxValueChanged(app, event)
            ...
            app.ComponentsListBox.scroll(value)
            components = findComponentPortsFromPortsInfo(value);
            app.SigInListBox.Items = components.inputPorts.PortName;
            app.SigOutListBox.Items = components.outputPorts.PortName;
        end
        ...
        % Clicked callback: ModelInListBox
        function ModelInListBoxClicked(app, event)
            item = event.InteractionInformation.Item;
            value = app.ModelInListBox.Items{item};
        ...
        % Clicked callback: ModelOutListBox
        function ModelOutListBoxClicked(app, event)
            item = event.InteractionInformation.Item;
            value = app.ModelOutListBox.Items{item};
        ...
        % Clicked callback: SigInListBox
        function SigInListBoxClicked(app, event)
            item = event.InteractionInformation.Item;
            value = app.SigInListBox.Items{item};

            app.SignalsListBox.Value = value;
            app.SignalsListBox.scroll(value)
        ...
        % Clicked callback: SigOutListBox
        function SigOutListBoxClicked(app, event)
            item = event.InteractionInformation.Item;
            value = app.SigOutListBox.Items{item};
        ...
        % Double-clicked callback: ComponentsListBox
        function ComponentsListBoxDoubleClicked(app, event)
            item = event.InteractionInformation.Item;
            value = app.ComponentsListBox.Items{item};
            % 记录日志，便于确认操作结果
            app.LogTextArea.Value = sprintf("正在打开模型:%s, 请稍后\n", value);
            open_system(value)
        end

        % Clicked callback: SignalsListBox
        function SignalsListBoxClicked(app, event)
            item = event.InteractionInformation.Item;
        ...
        function SearchButtonPushed(app, event)
            Sig = strtrim(string(app.SearchEditField.Value));
            allSignals = string(app.SignalsListBox.Items);

            % 若搜索为空，提示并退出
            if strlength(Sig) == 0
                uialert(app.UIFigure, '请输入要查询的信号名。', 'Search 提示');
                return;
            end

            if isempty(allSignals)
                uialert(app.UIFigure, 'Signals 列表为空，请先加载或生成 PortsInfo 数据。', ...
                    'Search 提示');
                return;
            end

            % 优先精确匹配（忽略大小写）
            hitIdx = find(strcmpi(allSignals, Sig), 1, 'first');

            % 精确匹配失败时，尝试部分匹配（contains）
            if isempty(hitIdx)
                partIdx = find(contains(lower(allSignals), lower(Sig)));

                if isempty(partIdx)
                    uialert(app.UIFigure, sprintf('未在 Signals 列表中找到匹配信号: %s', char(Sig)), ...
                        '未找到信号');
                    return;
                elseif numel(partIdx) == 1
                    hitIdx = partIdx(1);
                else
                    candidates = allSignals(partIdx);
                    [selIdx, ok] = listdlg( ...
                        'PromptString', sprintf('搜索 "%s" 匹配到多个信号，请选择一个：', char(Sig)), ...
                        'SelectionMode', 'single', ...
                        'ListString', cellstr(candidates), ...
                        'ListSize', [520, 360], ...
                        'Name', '选择信号');
                    if ok == 0 || isempty(selIdx)
                        return;
                    end
                    hitIdx = partIdx(selIdx);
                end
            end

            % 命中后联动刷新上下游模型
            value = char(allSignals(hitIdx));
            app.SearchEditField.Value = value;
            app.SignalsListBox.Value = value;
            app.SignalsListBox.scroll(value)
        ...
        % Button pushed function: SetModelPathButton
        function SetModelPathButtonPushed(app, event)
            % 优先使用当前输入框路径作为打开目录，否则退回到当前工作目录
            startPath = '';
            try
                startPath = char(string(app.ModelPathEditField.Value));
            catch
                startPath = '';
            end
            ...
            % 通过 UI 选择一个目录（取消时返回 0）
            modelPath = uigetdir(startPath, '请选择模型目录');
            if isequal(modelPath, 0)
                return;
            end

            % 目录有效性校验
            if ~isfolder(modelPath)
                uialert(app.UIFigure, sprintf('目录不存在: %s', char(modelPath)), '路径无效');
                return;
            end

            % 将目录及其子目录加入系统路径，便于可以找到相关模型
            ...
            % 记录日志，便于确认操作结果
            app.LogTextArea.Value = sprintf("模型路径已加入 MATLAB path:\n%s", modelPath);
        end

        % Clicked callback: EnumListListBox
        function EnumListListBoxClicked(app, event)
            item = event.InteractionInformation.Item;
            enumClass = app.EnumListListBox.Items{item};
            enumInfo = findEnumTypes('className', enumClass);
            app.EnumInfoTextArea.Value = enumInfo.details.DisplayText;
        end

        % Button pushed function: SetDCMButton
        function SetDCMButtonPushed(app, event)
            % 通过UI 打开文件，并将目录加到工作路径中，文件名显示到app.DCMEditField.Value 中
            [file, path] = uigetfile('*.dcm', '选择DCM文件');
            if isequal(file, 0)
                return;
            end
            app.DCMEditField.Value = file;
            app.LogTextArea.Value = sprintf("DCM文件已选择: %s", fullfile(path, file));
        end

        % Button pushed function: FindModelInfoButton
        function FindModelInfoButtonPushed(app, event)
            DCMfileName = app.DCMEditField.Value;
            R = createSubmodInfo('DCMfileName',DCMfileName);
            % 记录日志，便于确认操作结果
            app.LogTextArea.Value = sprintf("%s", R.summaryText);
        end
    end
    ...
    methods (Access = private)
        ...
        function createComponents(app)
            ...
            app.SourcePanel.Position = [18 11 514 172];
            ...
            app.PortsInfoExcelEditFieldLabel.Position = [7 81 82 22];
            ...
            app.PortsInfoExcelEditField.Position = [104 81 289 22];
            ...
            app.CreatePortsInfoExcelButton.Position = [231 9 146 23];
            ...
            app.CreateModelsButton.Position = [119 9 100 23];
            ...
            app.CreateGjsonButton.Position = [10 9 100 23];
            app.CreateGjsonButton.Text = 'CreateGjson';

            % Create ModelPathEditFieldLabel
            app.ModelPathEditFieldLabel = uilabel(app.SourcePanel);
            app.ModelPathEditFieldLabel.HorizontalAlignment = 'right';
            app.ModelPathEditFieldLabel.Position = [23 46 66 22];
            app.ModelPathEditFieldLabel.Text = 'Model Path';

            % Create ModelPathEditField
            app.ModelPathEditField = uieditfield(app.SourcePanel, 'text');
            app.ModelPathEditField.Position = [104 46 290 22];

            % Create SetModelPathButton
            app.SetModelPathButton = uibutton(app.SourcePanel, 'push');
            app.SetModelPathButton.ButtonPushedFcn = createCallbackFcn(app, @SetModelPathButtonPushed, true);
            app.SetModelPathButton.Tooltip = {'设置模型路径，便于双击直接打开模型'};
            app.SetModelPathButton.Position = [392 9 100 23];
            app.SetModelPathButton.Text = 'SetModelPath';

            % Create FindSignalPanel
            app.FindSignalPanel = uipanel(app.ArxmlTab);
            app.FindSignalPanel.Title = 'FindSignal';
            app.FindSignalPanel.Position = [18 194 474 335];
            ...
            app.SignalsListBoxLabel.Position = [22 237 44 22];
            ...
            app.SignalsListBox.ClickedFcn = createCallbackFcn(app, @SignalsListBoxClicked, true);
            app.SignalsListBox.Position = [14 23 212 210];
            ...
            app.ModelInListBoxLabel.Position = [242 142 48 22];
            ...
            app.ModelInListBox.ClickedFcn = createCallbackFcn(app, @ModelInListBoxClicked, true);
            app.ModelInListBox.Position = [308 25 150 139];
            ...
            app.ModelOutListBoxLabel.Position = [241 203 57 22];
            ...
            app.ModelOutListBox.ClickedFcn = createCallbackFcn(app, @ModelOutListBoxClicked, true);
            app.ModelOutListBox.Position = [313 192 144 35];

            % Create SearchEditFieldLabel
            app.SearchEditFieldLabel = uilabel(app.FindSignalPanel);
            app.SearchEditFieldLabel.HorizontalAlignment = 'right';
            app.SearchEditFieldLabel.Position = [22 272 43 22];
            app.SearchEditFieldLabel.Text = 'Search';

            % Create SearchEditField
            app.SearchEditField = uieditfield(app.FindSignalPanel, 'text');
            app.SearchEditField.Position = [80 272 266 22];

            % Create SearchButton
            app.SearchButton = uibutton(app.FindSignalPanel, 'push');
            app.SearchButton.ButtonPushedFcn = createCallbackFcn(app, @SearchButtonPushed, true);
            app.SearchButton.Position = [362 272 100 23];
            ...
            app.FindComponentPanel.Position = [503 194 583 335];
            ...
            app.SigOutListBoxLabel.Position = [373 277 42 22];
            app.SigOutListBoxLabel.Text = 'SigOut';

            % Create SigOutListBox
            app.SigOutListBox = uilistbox(app.FindComponentPanel);
            app.SigOutListBox.ClickedFcn = createCallbackFcn(app, @SigOutListBoxClicked, true);
            app.SigOutListBox.Position = [372 18 196 255];
            ...
            app.ComponentsListBoxLabel.Position = [216 277 73 22];
            ...
            app.ComponentsListBox.DoubleClickedFcn = createCallbackFcn(app, @ComponentsListBoxDoubleClicked, true);
            app.ComponentsListBox.Position = [216 18 148 254];
            ...
            app.SigInListBoxLabel.Position = [13 280 32 22];
            app.SigInListBoxLabel.Text = 'SigIn';

            % Create SigInListBox
            app.SigInListBox = uilistbox(app.FindComponentPanel);
            app.SigInListBox.ClickedFcn = createCallbackFcn(app, @SigInListBoxClicked, true);
            app.SigInListBox.Position = [10 18 195 257];
            ...
            app.TabGroup2.Position = [1 11 1099 527];
            ...
            app.TabGroup3.Position = [1 154 1021 372];
            ...
            app.delGcsAllButton.Position = [11 493 93 23];
            ...
            app.delUselessPortPanel.Position = [14 337 246 144];
            ...
            app.delLinesAttrPanel.Position = [17 216 243 105];
            ...
            app.ChangeModPortNamePanel.Position = [648 166 365 155];
            ...
            app.ChangeModSizePanel.Position = [650 373 365 144];
            ...
            app.ChangeAttrPanel.Position = [10 159 632 361];
            ...
            app.FindTab.Tooltip = {'模型对应DCM 文件，需要在matlab 路径中'};
            app.FindTab.Title = 'Find';

            % Create SystemEnumPanel
            app.SystemEnumPanel = uipanel(app.FindTab);
            app.SystemEnumPanel.Title = 'SystemEnum';
            app.SystemEnumPanel.Position = [709 172 285 345];

            % Create findEnumTypesButton
            app.findEnumTypesButton = uibutton(app.SystemEnumPanel, 'push');
            ...
            app.findEnumTypesButton.Position = [112 284 100 23];
            app.findEnumTypesButton.Text = 'findEnumTypes';

            % Create EnumListListBoxLabel
            app.EnumListListBoxLabel = uilabel(app.SystemEnumPanel);
            app.EnumListListBoxLabel.HorizontalAlignment = 'right';
            app.EnumListListBoxLabel.Position = [41 243 55 22];
            app.EnumListListBoxLabel.Text = 'EnumList';

            % Create EnumListListBox
            app.EnumListListBox = uilistbox(app.SystemEnumPanel);
            app.EnumListListBox.ClickedFcn = createCallbackFcn(app, @EnumListListBoxClicked, true);
            app.EnumListListBox.Position = [111 163 150 104];

            % Create EnumInfoTextAreaLabel
            app.EnumInfoTextAreaLabel = uilabel(app.SystemEnumPanel);
            app.EnumInfoTextAreaLabel.HorizontalAlignment = 'right';
            app.EnumInfoTextAreaLabel.Position = [40 124 56 22];
            app.EnumInfoTextAreaLabel.Text = 'EnumInfo';

            % Create EnumInfoTextArea
            app.EnumInfoTextArea = uitextarea(app.SystemEnumPanel);
            app.EnumInfoTextArea.Position = [111 12 150 136];

            % Create ModelInfoPanel
            app.ModelInfoPanel = uipanel(app.FindTab);
            app.ModelInfoPanel.Title = 'ModelInfo';
            app.ModelInfoPanel.Position = [386 170 302 350];

            % Create SetDCMButton
            app.SetDCMButton = uibutton(app.ModelInfoPanel, 'push');
            app.SetDCMButton.ButtonPushedFcn = createCallbackFcn(app, @SetDCMButtonPushed, true);
            app.SetDCMButton.Position = [35 246 100 23];
            app.SetDCMButton.Text = 'SetDCM';

            % Create ModelInfoTextArea
            app.ModelInfoTextArea = uitextarea(app.ModelInfoPanel);
            app.ModelInfoTextArea.Position = [27 19 260 208];

            % Create DCMEditFieldLabel
            app.DCMEditFieldLabel = uilabel(app.ModelInfoPanel);
            app.DCMEditFieldLabel.HorizontalAlignment = 'right';
            app.DCMEditFieldLabel.Position = [30 286 32 22];
            app.DCMEditFieldLabel.Text = 'DCM';

            % Create DCMEditField
            app.DCMEditField = uieditfield(app.ModelInfoPanel, 'text');
            app.DCMEditField.Position = [77 286 206 22];
            app.DCMEditField.Value = 'HB11_CCM_VP_V2.02_20260413_change.DCM';

            % Create FindModelInfoButton
            app.FindModelInfoButton = uibutton(app.ModelInfoPanel, 'push');
            app.FindModelInfoButton.ButtonPushedFcn = createCallbackFcn(app, @FindModelInfoButtonPushed, true);
            app.FindModelInfoButton.Position = [165 247 100 23];
            app.FindModelInfoButton.Text = 'FindModelInfo';
