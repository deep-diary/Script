        ...
        for i = 1:length(inList)
            ...
            try
                if fromTemplate
                    ...
                    % 创建Inport模块
                    bkIn = add_block(bkType, bkPath, ...
                        'MakeNameUnique','on',...
                        'Position', pos, ...
                        "BackgroundColor", BackgroundColor, ...
                        'OutDataTypeStr', DataType...
                        );
                    set_param(bkIn,"BackgroundColor","orange");
                end
        ...
        for i = 1:length(outList)
            try
                ...
                if fromTemplate
                    ...
                    Name = outList{i};
                    [DataType, Name] = findNameMdOut(Name, 'mode', mode);
                    type = 'Outport';
                    bkPath = [gcs '/' Name];
                    bkType = ['built-in/' type];
                    if strcmp(type,'Inport')
                    ...
                    % 创建Outport模块
                    bkOut = add_block(bkType, bkPath, ...
                        'MakeNameUnique','on',...
                        'Position', pos, ...
                        "BackgroundColor", BackgroundColor, ...
                        'OutDataTypeStr', DataType...
                        );
                    set_param(bkOut,"BackgroundColor","lightBlue");
                end
                ...
                posX = pos(1)-300;
                posY = pos(2);
                fromPos=[posX-15, posY, posX+15, posY+15]; % From 位置
                bkFrom = add_block('built-in/From', [gcs '/From'], ...
                                  'MakeNameUnique', 'on', 'Position', fromPos);
                set_param(bkFrom, 'GotoTag', Name);
                % 修改颜色，显示GotoTag Chenyaunfei
                set_param(bkFrom,'AttributesFormatString','%<GotoTag>');
                set_param(bkFrom,'BackgroundColor','yellow')
                % 连线
