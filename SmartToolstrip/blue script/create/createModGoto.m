...
%     addParameter(p,'isDelSuffix',false);      % 设置变量名和默认参数
    ...
    if isEnableIn
        ...
        for i = 1:sz(1)  % inports 是个cell(1950        3356),不能用length
            ...
            set_param(inputBlock,'AttributesFormatString','%<GotoTag>');
            set_param(inputBlock,'BackgroundColor','yellow')
    ...
    if isEnableOut
        ...
        for j = 1:length(Outport)
            ...
            % 修改颜色，显示GotoTag Chenyaunfei
            set_param(outputBlock,'AttributesFormatString','%<GotoTag>');
            set_param(outputBlock,'BackgroundColor','magenta')
